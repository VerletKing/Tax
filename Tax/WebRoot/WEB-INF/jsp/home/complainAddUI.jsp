...
<head>
   ...
   function doSelectDept(){
	   	var dept = $("#complainDeptID option:selected").val();
	   	if(dept!=""){
	   		$.ajax({
	   			type : "post",
	   			...
	   			success : function(data){
	   				if(data!=null && data!="" && data !=undefined){
	   					if(data.msg == "success"){
		   					var $name = $("#complainNameID");
		   					$name.empty();		   					$.each(data.userList,function(index,user){
		   						$name.append("<option name='" +user.userName+ "'>"+user.userName+ "</option>");
		   					})
	   					}else{
	   						alert("获取被投诉人列表失败。");   		
	   					}
...
<form id="form" name="form" action="" method="post" enctype="multipart/form-data">
    ...
    <table id="baseInfo" width="100%" align="center" class="list" border="0" cellpadding="0" cellspacing="0"  >
        ...
        <tr>
            ...
            <td><s:select id="complainDeptID" name="comp.toCompDept" list="#{'':'请选择','部门A':'部门A','部门B':'部门B'}" onchange="doSelectDept()"/></td>
        </tr>
        <tr>
            <td class="tdBg">被投诉人姓名：</td>
            <td>
	            <select id="complainNameID" name="comp.toCompName">
	            </select>
            </td>
        </tr>
        <tr>
		...
		<s:hidden name="comp.compName" value="%{#session.SYS_USER.userName}"/>
