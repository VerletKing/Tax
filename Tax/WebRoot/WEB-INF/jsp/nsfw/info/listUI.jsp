...
<title>信息发布管理</title>
	<script type="text/javascript">
		...
		function doAdd() {
			document.forms[0].action = "info_addUI.action";
			document.forms[0].submit();
		}

		function doEdit(id) {
			document.forms[0].action = "info_editUI.action?info.infoId=" + id;
			document.forms[0].submit();
		}

		function doDelete(id) {
			document.forms[0].action = "info_delete.action?info.infoId=" + id;
			document.forms[0].submit();
		}

		function doDeleteAll() {
			document.forms[0].action = "info_deleteSelected.action";
			document.forms[0].submit();
		}
		
		function doChange(infoId, state) {
			$.ajax({
				type : "POST",
				url : "info_publicInfo.action",
				data : {"info.infoId" : infoId , "info.state" : state},
				success : function(msg) {
					if ("更新状态成功" == msg) {
						if(state == 1){
							$("#show_"+infoId).html("发布");
							$("#oper_"+infoId).html('<a href="javascript:doChange(\'' +infoId+'\',0)">停用</a>');
						}else{
							$("#show_"+infoId).html("停用");
							$("#oper_"+infoId).html('<a href="javascript:doChange(\'' +infoId+'\',1)">发布</a>');
						}
					} else {
						alert("更新信息状态失败！");
					}
		...
		var list_ui = "info_listUI.action";
		function doSearch(){
			$("#pageNo").val(1);
						...
						<table width="100%" border="0">
							...
							<s:iterator value="pageResult.items" status="st">
								<tr <s:if test="#st.odd"> bgcolor="f8f8f8" </s:if>>
									...
									<td align="center"><span id="oper_<s:property value='infoId'/>"> 
										<s:if test="state==1">
											<a href="javascript:doChange('<s:property value='infoId'/>',0)">停用</a>
										</s:if>
										<s:else>
											<a href="javascript:doChange('<s:property value='infoId'/>',1)">发布</a>
										</s:else>
