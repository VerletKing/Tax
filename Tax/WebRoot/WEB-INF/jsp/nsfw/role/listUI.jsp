...
<head>
	...
	function doAdd(){
		document.forms[0].action="role_addUI.action";
		document.forms[0].submit();
	}
	
	function doEdit(id){
			document.forms[0].action="role_editUI.action?role.id="+id;
			document.forms[0].submit();
	}
	
	function doDelete(id){
			document.forms[0].action="role_delete.action?role.id="+id;
			document.forms[0].submit();
	}
	
	function doDeleteAll(){
			document.forms[0].action="role_deleteSelected.action";
			document.forms[0].submit();
	}
	
	var list_ui = "role_listUI.action";
	function doSearch(){
		$("#pageNo").val(1);
                            ...
                            <tr <s:if test="#st.odd">bgcolor="f8f8f8"</s:if>>
                                ...
                                <td align="center">
                                		<s:iterator value="rolePrivilege">
                                			<s:property value="#request.privilegeMap[id.code]"/>
                                		</s:iterator>
                                </td>
